...
with tab1:
    ...
    #'RestingECG'
    x7 = st.radio('Select Resting ECG', RestingECG_encoder.classes_)
    x7 = RestingECG_encoder.transform([x7])[0]

    #'MaxHR'
    x8 = st.slider('MaxHR', 0, 210, 140)

    #'ExerciseAngina'
    x9 = st.radio('Select Exercise Angina', ExerciseAngina_encoder.classes_)
    x9 = ExerciseAngina_encoder.transform([x9])[0]

    #'Oldpeak'
    x10 = st.slider('Oldpeak', -3.0, 6.5, 0.0)
